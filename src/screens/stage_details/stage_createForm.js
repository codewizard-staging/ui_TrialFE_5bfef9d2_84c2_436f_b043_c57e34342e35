...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, stageId, numfields;
        const mapItems = MapItems;

        let stage = row['stage'];

        numfields = Helper.GetAllNumberFields(stage);
        if (numfields.length > 0) Helper.UpdateNumberFields(stage, numfields);


        // Add Or Update Stage
        rslt = await Support.AddOrUpdateStage(stage, dropDownOptions, []);
        if (rslt.status) {
            stageId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Stage and navigation entity if it is deos not exist
            let navItem = stage.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "StageId", value: parseInt(stageId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateStage(data, dropDownOptions);
        ...
        global.AlertPopup("success", "Stage is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }

...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], stageId, stage, numfields;

        stage = row['stage'];
        stageId = row['stage'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) stageId = rslt.id;
        ...
        global.AlertPopup("success", "Stage is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
